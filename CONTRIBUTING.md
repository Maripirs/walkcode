...
- `src/data/lesson-records.js`, `drills.js`, `languages.js`, and `difficulty.js` together are the content source of truth. Keep explanations, walkthrough records, drills, language variants, and difficulty metadata in their matching file.
...
- Standalone drills belong in `extraCodeDrills` and need context, a full-code reveal, JavaScript and Python variants, and difficulty.
- A full-code context reveal is shown only when the entire completed exercise snippet is a real segment of a meaningfully larger solution and the answer does not appear elsewhere in that context. It must retain the focused line with a visibly highlighted `___`, show the actual surrounding code, and never fake context by repeating the preview. If that source is not available, adds no value, or leaks the answer, omit the reveal.
