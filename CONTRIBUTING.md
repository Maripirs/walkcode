...
- `src/data/lesson-records.js` is the single source of authored lesson records (`featured`): the original core records plus the roadmap problems upgraded from Work in progress to Built (authored via the `lesson()`/`ex()` helpers, which build code-fix exercises from the full solution). Each record needs JavaScript/Python code, input/output/example, concept choices, ordered steps, two code fixes, and guided complexity.
