...
const walkthroughPythonExercises = {
  'Design Twitter': [
    {"prompt":"Which line orders the gathered tweets newest-first?","correct":"all_tweets.sort(key=lambda t: t[0], reverse=True)","choices":["all_tweets.sort(key=lambda t: t[0], reverse=True)","all_tweets.sort(key=lambda t: t[0])","all_tweets.sort(key=lambda t: t[1], reverse=True)"],"why":"The timestamp is at index 0; sorting descending by it puts the most recent tweets first.","wrong":{"all_tweets.sort(key=lambda t: t[0])":"Ascending order returns the OLDEST tweets first.","all_tweets.sort(key=lambda t: t[1], reverse=True)":"That sorts by tweet id, unrelated to recency."}},
    {"prompt":"Which line returns the feed correctly?","correct":"return [tweet_id for _, tweet_id in all_tweets[:10]]","choices":["return [tweet_id for _, tweet_id in all_tweets[:10]]","return [ts for ts, _ in all_tweets[:10]]","return [tweet_id for _, tweet_id in all_tweets]"],"why":"Take the first ten and return each tweet id (the second element).","wrong":{"return [ts for ts, _ in all_tweets[:10]]":"That returns timestamps, not tweet ids.","return [tweet_id for _, tweet_id in all_tweets]":"That ignores the 10-item cap."}},
  ],
  'Swim in Rising Water': [
    {"prompt":"Which line sets the time-search range?","correct":"lo, hi = grid[0][0], n * n - 1","choices":["lo, hi = grid[0][0], n * n - 1","lo, hi = 0, n * n - 1","lo, hi = grid[0][0], n"],"why":"You cannot move before the start cell floods (lo = its elevation); the highest elevation is n²−1.","wrong":{"lo, hi = 0, n * n - 1":"Starting lo at 0 wastes steps below the unavoidable start elevation.","lo, hi = grid[0][0], n":"The maximum elevation is n²−1, not n."}},
    {"prompt":"Which line narrows toward the minimum feasible time?","correct":"if can_reach(grid, mid):","choices":["if can_reach(grid, mid):","if not can_reach(grid, mid):","if can_reach(grid, lo):"],"why":"When mid is feasible, keep it as the upper bound and search lower.","wrong":{"if not can_reach(grid, mid):":"That inverts the branch, raising hi on infeasible times.","if can_reach(grid, lo):":"Testing lo instead of mid never actually narrows the range."}},
  ],
  'Minimum Interval to Include Each Query': [
    {"prompt":"Which line tests whether an interval covers the query?","correct":"if start <= q <= end:","choices":["if start <= q <= end:","if start < q < end:","if start <= q or q <= end:"],"why":"A point is inside an interval when it is between the endpoints inclusive.","wrong":{"if start < q < end:":"Strict inequalities wrongly exclude a query sitting on an endpoint.","if start <= q or q <= end:":"or is true for almost every interval, ignoring real containment."}},
    {"prompt":"Which line keeps the smallest covering interval?","correct":"if best == -1 or size < best:","choices":["if best == -1 or size < best:","if best == -1 or size > best:","if size < best:"],"why":"Update best when nothing was found yet, or when this covering interval is smaller.","wrong":{"if best == -1 or size > best:":"That keeps the LARGEST covering interval, not the smallest.","if size < best:":"With best still -1 (no interval yet), size < -1 is never true, so the first cover is missed."}},
  ],
  'Maximum Profit in Job Scheduling': [
    {"prompt":"Which line finds the latest job that does not overlap job i?","correct":"if jobs[j][1] <= start:","choices":["if jobs[j][1] <= start:","if jobs[j][1] < start:","if jobs[j][0] <= start:"],"why":"A compatible job must finish at or before job i starts; scanning downward, the first such job is the latest.","wrong":{"if jobs[j][1] < start:":"A job ending exactly when i starts does not overlap, so <= is required.","if jobs[j][0] <= start:":"That compares the earlier job’s START, not its end."}},
    {"prompt":"Which line chooses between skipping and taking job i?","correct":"dp[i] = max(dp[i - 1] if i else 0, earned + (dp[prev] if prev >= 0 else 0))","choices":["dp[i] = max(dp[i - 1] if i else 0, earned + (dp[prev] if prev >= 0 else 0))","dp[i] = earned + (dp[prev] if prev >= 0 else 0)","dp[i] = max(dp[i - 1] if i else 0, earned)"],"why":"Taking job i earns its profit plus the best compatible prefix (dp[prev]); skipping keeps dp[i-1].","wrong":{"dp[i] = earned + (dp[prev] if prev >= 0 else 0)":"Always taking job i ignores the option of skipping it.","dp[i] = max(dp[i - 1] if i else 0, earned)":"Dropping dp[prev] forgets profit from earlier compatible jobs."}},
  ],
  'LRU Cache': [
    {"prompt":"In get, which line refreshes the key to most-recently used?","correct":"value = self.cache.pop(key)","choices":["value = self.cache.pop(key)","value = self.cache[key]","value = self.cache.get(key)"],"why":"pop removes the key so the following re-insert puts it at the back (most-recent).","wrong":{"value = self.cache[key]":"Reading without removing leaves the key in its old (stale) position.","value = self.cache.get(key)":"get also does not remove, so the key is not reordered."}},
    {"prompt":"Which line finds the least-recently-used key to evict?","correct":"oldest = next(iter(self.cache))","choices":["oldest = next(iter(self.cache))","oldest = list(self.cache)[-1]","oldest = key"],"why":"The first key in insertion order is the least recently used.","wrong":{"oldest = list(self.cache)[-1]":"The last key is the MOST recent, the opposite of what to evict.","oldest = key":"That is the key just inserted, not the oldest."}},
  ],
  'Word Ladder': [
    {"prompt":"Which line marks a neighbor visited so it is not revisited?","correct":"word_set.remove(candidate)","choices":["word_set.remove(candidate)","word_set.add(candidate)","nxt.append(word)"],"why":"Removing the candidate from the set ensures each word is enqueued only once.","wrong":{"word_set.add(candidate)":"Re-adding does nothing new and never prevents revisits.","nxt.append(word)":"That enqueues the current word again instead of the neighbor."}},
    {"prompt":"Which line returns the shortest ladder length?","correct":"if word == end_word:","choices":["if word == end_word:","if word == begin_word:","if end_word in word_set:"],"why":"BFS reaches end_word first along a shortest path, so return the current step count then.","wrong":{"if word == begin_word:":"That returns at the start, giving length 1.","if end_word in word_set:":"end_word is in the set the whole time; this returns on the first word."}},
  ],
  'Detect Squares': [
    {"prompt":"Which line keeps only valid diagonal corners?","correct":"if abs(px - x) != abs(py - y) or px == x:","choices":["if abs(px - x) != abs(py - y) or px == x:","if abs(px - x) == abs(py - y) or px == x:","if abs(px - x) != abs(py - y):"],"why":"A square’s diagonal has equal horizontal and vertical gaps, and a zero gap (px == x) is degenerate.","wrong":{"if abs(px - x) == abs(py - y) or px == x:":"That skips exactly the valid diagonals.","if abs(px - x) != abs(py - y):":"Dropping the px == x guard counts zero-area degenerate squares."}},
    {"prompt":"Which line counts squares for this diagonal corner?","correct":"total += cnt * self.counts.get((x, py), 0) * self.counts.get((px, y), 0)","choices":["total += cnt * self.counts.get((x, py), 0) * self.counts.get((px, y), 0)","total += cnt","total += self.counts.get((x, py), 0) + self.counts.get((px, y), 0)"],"why":"A square needs the diagonal corner AND both side corners, so multiply all three counts.","wrong":{"total += cnt":"That ignores whether the two side corners even exist.","total += self.counts.get((x, py), 0) + self.counts.get((px, y), 0)":"Adding the side corners counts them separately instead of as one square."}},
  ],
  'Find Median from Data Stream': [
    {"prompt":"Which line inserts the number in sorted order?","correct":"bisect.insort(self.nums, num)","choices":["bisect.insort(self.nums, num)","self.nums.append(num)","self.nums.sort()"],"why":"bisect.insort finds the sorted position and inserts there in one step.","wrong":{"self.nums.append(num)":"Appending breaks the sorted order the median relies on.","self.nums.sort()":"Sorting after a bare append is O(n log n) per add and here num was never added."}},
    {"prompt":"Which line returns the median for an even count?","correct":"return (self.nums[mid - 1] + self.nums[mid]) / 2","choices":["return (self.nums[mid - 1] + self.nums[mid]) / 2","return self.nums[mid]","return (self.nums[mid] + self.nums[mid + 1]) / 2"],"why":"An even count averages the two central elements at mid-1 and mid.","wrong":{"return self.nums[mid]":"A single element is the odd-count case.","return (self.nums[mid] + self.nums[mid + 1]) / 2":"For even n, the two middles are mid-1 and mid, not mid and mid+1."}},
  ],
  'Design Add and Search Words Data Structure': [
    {"prompt":"Which line handles a normal (non-dot) character?","correct":"return dfs(node[\"children\"][char], i + 1) if char in node[\"children\"] else False","choices":["return dfs(node[\"children\"][char], i + 1) if char in node[\"children\"] else False","return dfs(node[\"children\"][char], i + 1)","return char in node[\"children\"]"],"why":"A literal follows its one edge if it exists, then matches the rest of the word.","wrong":{"return dfs(node[\"children\"][char], i + 1)":"Indexing a missing child raises a KeyError.","return char in node[\"children\"]":"Stopping at the edge ignores the remaining characters."}},
    {"prompt":"For a \".\", which line tries every child?","correct":"if dfs(node[\"children\"][key], i + 1):","choices":["if dfs(node[\"children\"][key], i + 1):","if dfs(node[\"children\"][char], i + 1):","if dfs(node[\"children\"][key], i):"],"why":"A dot matches any letter, so recurse into each child key, advancing the index by one.","wrong":{"if dfs(node[\"children\"][char], i + 1):":"char is \".\", which is not a real child key.","if dfs(node[\"children\"][key], i):":"Not advancing i re-tests the same dot forever."}},
  ],
  'Interleaving String': [
    {"prompt":"Which line takes the next character from s1?","correct":"if i < len(s1) and s1[i] == s3[k] and dfs(i + 1, j):","choices":["if i < len(s1) and s1[i] == s3[k] and dfs(i + 1, j):","if i < len(s1) and s1[i] == s3[i] and dfs(i + 1, j):","if s1[i] == s3[k] and dfs(i + 1, j):"],"why":"When s1’s next char equals s3[k] (k = i + j) and the rest interleaves, consuming from s1 works.","wrong":{"if i < len(s1) and s1[i] == s3[i] and dfs(i + 1, j):":"s3 is indexed by k = i + j, not i.","if s1[i] == s3[k] and dfs(i + 1, j):":"Without the bounds guard, s1[i] reads past the end."}},
    {"prompt":"Which line rejects mismatched lengths up front?","correct":"if len(s1) + len(s2) != len(s3):","choices":["if len(s1) + len(s2) != len(s3):","if len(s1) + len(s2) == len(s3):","if len(s1) != len(s2):"],"why":"An interleaving uses every character of both, so the lengths must sum to |s3|.","wrong":{"if len(s1) + len(s2) == len(s3):":"That rejects the only cases that could possibly work.","if len(s1) != len(s2):":"s1 and s2 need not be equal length."}},
  ],
  'Longest Increasing Path in a Matrix': [
    {"prompt":"Which line extends the path to a valid neighbor?","correct":"best = max(best, 1 + dfs(next_r, next_c))","choices":["best = max(best, 1 + dfs(next_r, next_c))","best = max(best, dfs(next_r, next_c))","best = 1 + dfs(next_r, next_c)"],"why":"Adding one to a strictly-larger neighbor’s best extends the increasing path; keep the max.","wrong":{"best = max(best, dfs(next_r, next_c))":"Dropping the + 1 forgets the step onto the neighbor.","best = 1 + dfs(next_r, next_c)":"Overwriting loses a longer path found via another neighbor."}},
    {"prompt":"Which line seeds a cell’s path length?","correct":"best = 1","choices":["best = 1","best = 0","best = dfs(r, c)"],"why":"A single cell is already an increasing path of length 1.","wrong":{"best = 0":"Length 0 undercounts every path by one.","best = dfs(r, c)":"Calling dfs on the same cell recurses forever."}},
  ],
  'Edit Distance': [
    {"prompt":"Which line handles two matching characters?","correct":"dp[i][j] = dp[i - 1][j - 1]","choices":["dp[i][j] = dp[i - 1][j - 1]","dp[i][j] = 1 + dp[i - 1][j - 1]","dp[i][j] = dp[i - 1][j]"],"why":"Matching characters need no edit, so the cost equals the diagonal (both prefixes one shorter).","wrong":{"dp[i][j] = 1 + dp[i - 1][j - 1]":"Adding 1 charges an edit even though the characters already match.","dp[i][j] = dp[i - 1][j]":"That is the delete neighbor, not the no-cost diagonal."}},
    {"prompt":"Which line handles a mismatch?","correct":"dp[i][j] = 1 + min(dp[i - 1][j], dp[i][j - 1], dp[i - 1][j - 1])","choices":["dp[i][j] = 1 + min(dp[i - 1][j], dp[i][j - 1], dp[i - 1][j - 1])","dp[i][j] = min(dp[i - 1][j], dp[i][j - 1], dp[i - 1][j - 1])","dp[i][j] = 1 + max(dp[i - 1][j], dp[i][j - 1], dp[i - 1][j - 1])"],"why":"One edit (delete/insert/replace) plus the cheapest of the three neighboring subproblems.","wrong":{"dp[i][j] = min(dp[i - 1][j], dp[i][j - 1], dp[i - 1][j - 1])":"Dropping the + 1 never charges for the required edit.","dp[i][j] = 1 + max(dp[i - 1][j], dp[i][j - 1], dp[i - 1][j - 1])":"Taking the max gives the most expensive, not the minimum, edit count."}},
  ],
  'Burst Balloons': [
    {"prompt":"Which line scores bursting mid last in this interval?","correct":"dp[left][right] = max(dp[left][right], coins[left] * coins[mid] * coins[right] + dp[left][mid] + dp[mid][right])","choices":["dp[left][right] = max(dp[left][right], coins[left] * coins[mid] * coins[right] + dp[left][mid] + dp[mid][right])","dp[left][right] = max(dp[left][right], coins[mid - 1] * coins[mid] * coins[mid + 1] + dp[left][mid] + dp[mid][right])","dp[left][right] = max(dp[left][right], coins[left] * coins[mid] * coins[right] + dp[left][right])"],"why":"If mid bursts last, its neighbors are the fixed boundaries left and right, plus the two solved sub-intervals.","wrong":{"dp[left][right] = max(dp[left][right], coins[mid - 1] * coins[mid] * coins[mid + 1] + dp[left][mid] + dp[mid][right])":"Using mid’s immediate neighbors assumes it bursts FIRST — the trick is that it bursts last.","dp[left][right] = max(dp[left][right], coins[left] * coins[mid] * coins[right] + dp[left][right])":"Adding dp[left][right] to itself is circular and ignores the sub-intervals."}},
    {"prompt":"Which line sets up the boundary balloons?","correct":"coins = [1] + nums + [1]","choices":["coins = [1] + nums + [1]","coins = list(nums)","coins = [0] + nums + [0]"],"why":"Sentinel balloons of value 1 at both ends give every real balloon well-defined neighbors.","wrong":{"coins = list(nums)":"Without sentinels, edge balloons have no left/right neighbor to multiply.","coins = [0] + nums + [0]":"Value-0 sentinels would zero out edge products, undercounting coins."}},
  ],
  'Regular Expression Matching': [
    {"prompt":"Which line matches a normal (non-star) token?","correct":"dp[i][j] = (p[j - 1] in (s[i - 1], \".\")) and dp[i - 1][j - 1]","choices":["dp[i][j] = (p[j - 1] in (s[i - 1], \".\")) and dp[i - 1][j - 1]","dp[i][j] = (p[j - 1] in (s[i - 1], \".\")) or dp[i - 1][j - 1]","dp[i][j] = p[j - 1] in (s[i - 1], \".\")"],"why":"A literal or \".\" matches one char, so it needs the character to match AND the shorter prefixes (the diagonal).","wrong":{"dp[i][j] = (p[j - 1] in (s[i - 1], \".\")) or dp[i - 1][j - 1]":"or would call it a match even when the characters differ.","dp[i][j] = p[j - 1] in (s[i - 1], \".\")":"Ignoring the diagonal forgets whether everything before matched."}},
    {"prompt":"For a star, which line checks the repeated token against the current char?","correct":"matches = p[j - 2] in (s[i - 1], \".\")","choices":["matches = p[j - 2] in (s[i - 1], \".\")","matches = p[j - 1] in (s[i - 1], \".\")","matches = p[j - 2] == s[i - 1] == \".\""],"why":"A \"*\" repeats the token immediately before it, at pattern index j − 2.","wrong":{"matches = p[j - 1] in (s[i - 1], \".\")":"p[j-1] is the \"*\" itself, not the token it repeats.","matches = p[j - 2] == s[i - 1] == \".\"":"That requires the char to equal both s[i-1] and \".\", which is impossible."}},
  ],
  'Sliding Window Maximum': [
    {"prompt":"Which line keeps the deque decreasing?","correct":"while deque and nums[deque[-1]] < nums[right]:","choices":["while deque and nums[deque[-1]] < nums[right]:","while deque and nums[deque[-1]] > nums[right]:","while deque and nums[deque[0]] < nums[right]:"],"why":"A value smaller than the incoming one can never be a future max while it stays, so pop such indexes from the back.","wrong":{"while deque and nums[deque[-1]] > nums[right]:":"Popping LARGER values destroys the very maxima you need.","while deque and nums[deque[0]] < nums[right]:":"The comparison must happen at the back (index -1), not the front."}},
    {"prompt":"Which line drops an index that slid out of the window?","correct":"deque.popleft()","choices":["deque.popleft()","deque.pop()","deque.append(left)"],"why":"The expired index sits at the front, so remove it from the front.","wrong":{"deque.pop()":"pop() removes from the back, not the expired front.","deque.append(left)":"Appending grows the deque instead of trimming the front."}},
  ],
  'Merge K Sorted Lists': [
    {"prompt":"Which line pairs the lists for merging?","correct":"merged.append(merge_two(lists[i], second))","choices":["merged.append(merge_two(lists[i], second))","merged.append(merge_two(lists[i], lists[i]))","merged.append(lists[i])"],"why":"Each round merges list i with its neighbor (captured in second), halving the count.","wrong":{"merged.append(merge_two(lists[i], lists[i]))":"Merging a list with itself duplicates nodes into a cycle.","merged.append(lists[i])":"That just copies lists without merging them."}},
    {"prompt":"In the two-list merge, which line takes the smaller head?","correct":"if a.val <= b.val:","choices":["if a.val <= b.val:","if a.val >= b.val:","if a.val < b.val:"],"why":"The next node of a merged sorted list is the smaller of the two current heads (<= keeps it stable).","wrong":{"if a.val >= b.val:":"Taking a when it is larger produces a non-sorted list.","if a.val < b.val:":"A strict < mishandles equal values at the branch boundary here."}},
  ],
  'Median of Two Sorted Arrays': [
    {"prompt":"Which line guarantees you search the shorter array?","correct":"if len(a) > len(b):","choices":["if len(a) > len(b):","if len(a) < len(b):","if len(a) == len(b):"],"why":"Swapping when a is longer keeps the binary search over the smaller array’s cut range.","wrong":{"if len(a) < len(b):":"That swaps when a is already the shorter one — backwards.","if len(a) == len(b):":"Equal lengths need no swap; this misses the long-a case."}},
    {"prompt":"Which line recognizes a correct partition?","correct":"if a_left <= b_right and b_left <= a_right:","choices":["if a_left <= b_right and b_left <= a_right:","if a_left <= a_right and b_left <= b_right:","if a_left <= b_right or b_left <= a_right:"],"why":"A correct cut has every left element ≤ every right element ACROSS the two arrays.","wrong":{"if a_left <= a_right and b_left <= b_right:":"Comparing within each array is always true and proves nothing about the cut.","if a_left <= b_right or b_left <= a_right:":"or accepts half-valid cuts where one side is still out of order."}},
  ],
  'Reverse Nodes in K-Group': [
    {"prompt":"After reversing, which line advances to the next group?","correct":"group_prev = old_front","choices":["group_prev = old_front","group_prev = prev","group_prev = kth"],"why":"The group’s original first node is now its last, so it becomes the boundary before the next group.","wrong":{"group_prev = prev":"prev is the new front of the group, not the boundary after it.","group_prev = kth":"kth was the old last node, already moved by the reversal."}},
    {"prompt":"Which line records where the next group begins before reversing?","correct":"group_next = kth.next","choices":["group_next = kth.next","group_next = kth","group_next = group_prev.next"],"why":"The node just past the kth is where the reversal should stop and the next group starts.","wrong":{"group_next = kth":"kth is the last node OF the group, not the one after it.","group_next = group_prev.next":"That is the group’s first node, the wrong boundary."}},
  ],
  'Serialize and Deserialize Binary Tree': [
    {"prompt":"Which line serializes a subtree in preorder?","correct":"return str(node.val) + ',' + visit(node.left) + ',' + visit(node.right)","choices":["return str(node.val) + ',' + visit(node.left) + ',' + visit(node.right)","return visit(node.left) + ',' + str(node.val) + ',' + visit(node.right)","return str(node.val) + ',' + visit(node.right) + ',' + visit(node.left)"],"why":"Preorder emits the value first, then the left subtree, then the right — the order deserialize expects.","wrong":{"return visit(node.left) + ',' + str(node.val) + ',' + visit(node.right)":"That is in-order, which does not uniquely determine the tree.","return str(node.val) + ',' + visit(node.right) + ',' + visit(node.left)":"Right-before-left would rebuild a mirror image."}},
    {"prompt":"Which line rebuilds a node from a token?","correct":"node = TreeNode(int(value))","choices":["node = TreeNode(int(value))","node = TreeNode(value)","node = value"],"why":"The token is a string, so convert it to an int for the node’s value.","wrong":{"node = TreeNode(value)":"A string value would break numeric comparisons later.","node = value":"That stores the raw string, not a tree node."}},
  ],
  'Reconstruct Itinerary': [
    {"prompt":"Which line orders destinations so pop() gives the smallest?","correct":"destinations.sort(reverse=True)","choices":["destinations.sort(reverse=True)","destinations.sort()","destinations.reverse()"],"why":"pop() removes the last element, so a descending sort makes it return the lexically smallest.","wrong":{"destinations.sort()":"Ascending order makes pop() return the LARGEST, breaking the tie rule.","destinations.reverse()":"Reversing an unsorted list gives no ordering guarantee."}},
    {"prompt":"Which line records an airport in the route?","correct":"route.append(airport)","choices":["route.append(airport)","route.insert(0, airport)","return airport"],"why":"Hierholzer appends an airport once it is stuck; reversing the collected list yields the itinerary.","wrong":{"route.insert(0, airport)":"Prepending each airport defeats the append-then-reverse pattern and is slower.","return airport":"Returning here abandons the traversal before all edges are used."}},
  ],
  'Alien Dictionary': [
    {"prompt":"Which line rejects the impossible prefix case?","correct":"if len(first_word) > len(second_word) and first_word.startswith(second_word):","choices":["if len(first_word) > len(second_word) and first_word.startswith(second_word):","if first_word.startswith(second_word):","if len(first_word) > len(second_word):"],"why":"A word cannot sort before its own prefix, so a longer word starting with the next word is invalid.","wrong":{"if first_word.startswith(second_word):":"Equal words are fine; the length check is required.","if len(first_word) > len(second_word):":"Longer-first is only invalid when it is also a prefix of the shorter word."}},
    {"prompt":"Which line captures the ordering between two letters?","correct":"if first != second:","choices":["if first != second:","if first == second:","if first < second:"],"why":"The first position where the words differ gives the single ordering edge; then break.","wrong":{"if first == second:":"Equal characters reveal no ordering.","if first < second:":"The existing alphabet order is irrelevant; any first difference defines the edge."}},
  ],
  'Word Search II': [
    {"prompt":"Which line inserts a character node into the trie?","correct":"node = node.children.setdefault(char, TrieNode())","choices":["node = node.children.setdefault(char, TrieNode())","node = node.children[char]","node.children[char] = word"],"why":"setdefault creates the child if missing and steps into it, so shared prefixes share a path.","wrong":{"node = node.children[char]":"Indexing a missing child raises a KeyError.","node.children[char] = word":"That stores the word instead of building child nodes."}},
    {"prompt":"Which line prevents reporting the same word twice?","correct":"node.word = None","choices":["node.word = None","node.word = found","board[r][c] = None"],"why":"Clearing the terminal marker after recording it means a second path to the same word adds nothing.","wrong":{"node.word = found":"That restores the word, making it reportable again.","board[r][c] = None":"That corrupts the board instead of the trie marker."}},
  ],
