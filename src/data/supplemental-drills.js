...
export const supplementalCodeDrills = [
  ...
  d('Merge K Sorted Lists', 'Heap / Priority Queue', 'Hard', 'mergeTwo splices two sorted lists by repeatedly taking the smaller head; mergeKLists folds the k lists together in pairs until one remains.',
    { prompt: 'After one list is exhausted, which line appends the remaining sorted nodes?', correct: 'tail.next = a || b;', choices: ['tail.next = a || b;', 'tail.next = a && b;', 'tail.next = null;'], why: 'Whichever list still has nodes is already sorted, so attach it whole.' },
    { prompt: 'After one list is exhausted, which line appends the remaining sorted nodes?', correct: 'tail.next = a if a else b', choices: ['tail.next = a if a else b', 'tail.next = a and b', 'tail.next = None'], why: 'Whichever list still has nodes is already sorted, so attach it whole.' }),
