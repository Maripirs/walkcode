...
export const descriptionsByTitle = {
  ...
  'Merge Two Sorted Lists': 'You are given the heads of two linked lists, each already sorted in increasing order. Return one sorted list that contains every node from both — by re-linking the existing nodes, not creating new ones. Because both inputs are already sorted, the next node of the result is always whichever of the two current heads is smaller. Either list may be empty.',
  'Diameter of Binary Tree': 'The diameter of a binary tree is the length of the longest path between any two nodes, counted in edges. That path need not pass through the root — it is simply the longest chain of connected nodes anywhere in the tree (for example, up from one leaf, through a shared ancestor, and down to another leaf). Return that maximum number of edges.',
};
